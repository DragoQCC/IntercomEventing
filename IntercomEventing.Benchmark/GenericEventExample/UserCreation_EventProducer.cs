...
public class UserCreation_EventProducer
{
    ...
    {
        await UserCreatedEvent.RaiseEvent(new UserCreatedEventCall(user),this);
    }
    
    public async Task NotifyUserThresholdReachedAsync(User user)
    {
        await UserThresholdReachedEvent.RaiseEvent(new UserThresholdReachedEventCall(user));
    }
}